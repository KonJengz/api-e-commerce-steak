...
- ถ้าเรียกจาก browser และต้องการให้ cookie ทำงาน ต้องใช้ `credentials: "include"` ใน `fetch` หรือ `withCredentials: true` ใน Axios โดยเฉพาะ `verify-email`, `login`, `google/login`, `github/login`, `google/link/start`, `github/link/start`, `oauth/exchange`, `refresh`, `logout`
